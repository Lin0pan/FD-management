 ...
 *
 * The one status change that frees something (FR-4), which is what makes it the answer to a waiting
 * list rather than a tidy-up. Everything else stays, the number included, so the archived record
 * still says which slot it held (PRD §7, ADR-010). There is no way back out: a returning household is
 * registered anew (FR-7, US-11).
 *
 * Because it is irreversible and always a judgement, the reason is mandatory and is the record of it.
 * Both that and the legality of the move are {@link transition}'s to decide, and they are different
 * errors. Archiving a *blocked* customer is legal — a paused household can still leave.
 */
...
/** Exactly what the repository writes; the reason itself is the entry's `why`. */
 ...
 * @throws {IllegalStatusTransition} if already archived — their slot may by now be someone else's, so
 *   archiving it again is not a no-op.
...
export async function archiveCustomer(
  ...
  // Both questions settled before anything is written — the write below frees the number, so a
  // refusal here must leave the slot where it was.
