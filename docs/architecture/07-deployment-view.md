...
The isolated server exists for the specs that state something the **whole register** decides. The
first of those is the **quota**: it may not fall below the active customer count, so on a shared
database holding customers on numbers in the hundreds no quota could ever leave the register full —
nor leave one _group_ full, which since US-31 is a second way a register can run out (an even number
...
| `FD_FIXED_NOW_FILE` | Playwright `webServer.env` only                                            | unset                                                 | No       | Names a file holding an ISO instant that `systemClock` returns instead of the wall clock. Read from the environment **once at module load**, so a running deployment cannot acquire a fixed clock; the file itself is re-read on every call, so a spec can move "today" without restarting the server. Unreadable content falls back to the wall clock |
...
| Seed policy values       | `npm run db:seed`                               | Idempotent; writes no audit entry. Provisional: quota 240, €2.00 and €1.00 per head, a €5.00 cap — the seeded quota is **not** DF's real one (see below). **Real:** the egg rule, 3 → 6, 5 → 12, 8 → 18, which DF confirmed. All of them are changed on `/einstellungen` |
