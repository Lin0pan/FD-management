...
flowchart TB
    subgraph node["FD's machine — the only production node"]
        ...
        subgraph proc["Node 22 process — npm start"]
...
| Node version        | `.nvmrc`, and `engines` in `package.json`                     | 22                                                    | **Yes**  | CI reads `node-version-file: .nvmrc`; the app is not tested on anything else                                                                                                                                                                                                                                                                                                      |
