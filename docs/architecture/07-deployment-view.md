...
| Seed policy values       | `npm run db:seed`                               | Idempotent; writes no audit entry. Provisional: quota 240, €2.00 and €1.00 per head, a €5.00 cap, Thursday, anchored at `2026-W02` RED — the seeded quota is **not** DF's real one (see below). **Real:** the egg rule, 3 → 6, 5 → 12, 8 → 18, which DF confirmed. All of them are changed on `/einstellungen` |
