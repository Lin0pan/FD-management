...
### `src/domain/` — 28 modules in four families
...
| `distribution/`         | What happens at an afternoon that took place: the session itself and the groups it serves (`session.ts`), the counter verdict and its fixed precedence, one hand-out per session, the customer balance and the amount to collect, the served/expected tally, consecutive no-shows, and week-colour alternation from a single anchor — which since [ADR-020](adr/020-the-distribution-session-not-the-calendar-is-what-a-hand-out-belongs-to.md) decides nothing and is removed in US-36 |
...
| `DistributionRecordRepository`  | Hand-outs for a customer and for a session, create, amend the amount handed over, remove                                                                                                                                                                                                                         |
