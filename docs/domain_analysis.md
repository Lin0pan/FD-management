...
| Price                       | Small amount the customer pays to receive food. **Derived per head**: a fixed price per grown-up and a fixed price per child, multiplied by the two counts — but never more than the **Maximalpreis**, an optional upper limit on what one household pays for one distribution, so a large household stops there. Unlike the portion allowance, it does _not_ flex with supply or special occasions. |
...
| Attribute                  | Description                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| -------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Card number                | `<customer number>` + `k` + a running index, e.g. the cards printed under customer number 50 are numbered `50k1`, `50k2`, `50k3`, ... Only the most recently issued card number is valid; earlier ones are invalidated on reissue. A card number is **issued once and never reused**: the index counts the cards of the _number_, across every household that has ever held it, so a household taking over a slot an archive freed starts above the card the previous household walked away with (US-25). |
...
7. Customer pays the small price for their household: the price per grown-up times the grown-up
   count plus the price per child times the children count — capped at the **Maximalpreis** if one
   is configured. A household of four grown-ups and three children owes 11,00 € per head but pays
   5,00 €, because that is FD's Maximalpreis. The portion allowance is unaffected: the cap caps
   money, not food.
...
1. **Concrete policy values:** The actual numbers are still unknown — the price per grown-up and per
   child, the Maximalpreis, the standard portions per grown-up and per child, and the customer quota
   `N`. Needed as initial configuration data, not as structure. FD have since named three of them
   (2,00 € per grown-up, 1,00 € per child, a Maximalpreis of 5,00 €), and they are seeded rather
   than coded, which is what leaves the rest safe to stay open.
