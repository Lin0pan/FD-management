...
- **Business rules are configurable data, not constants.** The prices per head, the Maximalpreis,
  the quota `N` and the egg allowance live in the database, editable in the UI — never hard-coded. A
  change applies immediately; superseded versions are kept as history.
